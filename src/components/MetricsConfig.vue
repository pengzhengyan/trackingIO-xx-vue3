...
import { getAsaMetrics } from '@/api/index.js'
import { useMetrics } from '@/pinia/metrics'

const metricsStore = useMetrics()
const props = defineProps(['title', 'requestAsaData'])
  ...
  getAsaMetrics().then((res: any) => {
    ...
    data.forEach((item: any, index: number) => {
      ...
      item.children.forEach((sub: submenuItem) => {
        ...
        textKey.push({ text: sub.text, key: sub.key })
        // 检查子menu的key是否包含于 已经选指标 中
        const isActive = checkedMetrics.value.includes(sub.key)
      ...
      sidebarMenu.push(obj)
    })
    // 将asa text-key同步到pinia
    metricsStore.changeAsaTextKey(textKey)
  })
})
...
const iconList: string[] = reactive(["Grid", "Switch", "Coin", "Guide", "TrendCharts"])
/** 
 * 指标的menu的内容
 * children放着实际指标内容
 */
const sidebarMenu = reactive<menuItem[]>([])
...
const subMenuStatus = computed(() => {
  ...
  sidebarMenu.forEach((item: menuItem) => {
    const list: boolean[] = []
...
const mainMenuStatus = computed(() => {
  ...
  sidebarMenu.forEach((item, index) => {
    const isShow = item.text.includes(searchContent.value.toUpperCase()) || subMenuStatus.value[index].includes(true)
    list.push(isShow)
  })
  return list
})
...
const handleMenuSelect = (index: string) => {
  const found = sidebarMenu?.find((item: menuItem) => item.index === index)?.value
...
// checkbox-group全选回调
const handleCheckAllChange = (index: string, isCheckAll: boolean) => {
  const foundIndex = sidebarMenu.findIndex(item => item.index === index)
  // 全选状态发生变化的一组menu
  const onChangeMenu = sidebarMenu[foundIndex]
  ...
  if (onChangeMenu.checkedList.length === 0 && onChangeMenu.children.some((sub) => sub.text === fixedItem)) {
...
const updateCheckboxStatus = (changedItem: string) => {
  const changedKey = textKey.find(ele => ele.text === changedItem).key
  sidebarMenu.some((item: menuItem) => {
 ...
 */
const handleSubmit = () => {
  const list = selectedItems.value.map(item => textKey.find(ele => ele.text === item).key)
  metricsStore.changeAsaCheckedMetrics(list)
      ...
      <template #header="{ close }">
        <div class="my-header">
          <div class="dialog-title">
            ...
            <div class="searchbox">
              <el-input placeholder="输入指标关键词"
                        v-model.trim="searchContent"></el-input>
            </div>
      ...
      <div class="custom-target-content">
        ...
        <div class="custom-left">
          <el-menu default-active="1"
            ...
            <el-menu-item v-for="(item, index) in sidebarMenu"
        ...
        <div class="custom-centre">
          <div class="bese-target-box"
               id="targetBox">
            <div class="bese-target-item"
              ...
              <el-checkbox v-model="item.checkAll"
                           ...
                           :disabled='index > 2'>{{ item.text }}</el-checkbox>
              <el-checkbox-group v-model="item.checkedList"
                                 @change="handleCheckedListChange(item.index)"
                                 :text-color='activeColor'>
                <el-checkbox v-for="(subItem, subIndex) in item.children"
        ...
        <div class="custom-right">
          <div class="custom-title">
            ...
            <span class="custom-title-rg"
                  @click="cleanupSelected">清空全部</span>
          </div>
              ...
              <template #item="{ element }">
                <div class="choose-item">
                  <span class="choose-item-lf">
                    <el-icon v-if="element === firstItem"
    ...
    .my-header {
      ...
      .dialog-title {
        ...
        .el-dialog__headerbtn {
          background: #fff;
          -webkit-box-shadow: 0 8px 16px rgba(55, 66, 152, .2);
          box-shadow: 0 8px 16px rgba(55, 66, 152, .2);
          border-radius: 8px;
          width: 40px;
          height: 40px;
          top: -12px;
